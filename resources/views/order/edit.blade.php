                ...
                <form action="{{ route('order.update',$order->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="order_no" id="order_no" class="form-control" placeholder="Enter Order No" value="{{$order->order_no}}">
                        </div>
                        <!-- PO No -->
                        <div class="col-md-3">
                            <label for="po_no" class="form-label">PO No</label>
                            <input type="text" name="po_no" id="po_no" value="{{$order->po_no}}" class="form-control" placeholder="Enter PO No">
                        </div>
                        <!-- Order Month -->
                        <div class="col-md-3">
                            <label for="order_month" class="form-label">Order Month</label>
                            <input type="month" name="order_month" value="{{$order->order_month}}" id="order_month" class="form-control">
                        </div>
                    </div>
                                 ...
                                 @foreach ($order->order_product as $detail)
                                    <tr data-index="{{$loop->iteration}}">
                                        ...
                                        <td><input class="form-control" type="number" value="{{$detail->price_unit ?? 1}}"
                                                name="products[{{$detail->product->id ?? ''}}] "></td>
                                        <td><input class="form-control" type="number" value="{{$detail->sst_percent ?? 1}}"
                                                name="products[{{$detail->product->id ?? ''}}][sst_percent]"></td>
                                        <td><input class="form-control" type="number" value="{{$detail->sst_value ?? 1}}"
                                                name="products[{{$detail->product->id ?? ''}}][sst_value]"></td>
                                        <td><input class="form-control" type="number" value="{{$detail->firm_qty ?? 1}}"
                                                name="products[{{$detail->product->id ?? ''}}][firm_qty]"></td>
                                        <td><input class="form-control" type="number" value="{{$detail->forecast_qty_1 ?? 1}}"
                                                name="products[{{$detail->product->id ?? ''}}][forecast_qty_1]"></td>
                                        <td><input class="form-control" type="number" value="{{$detail->forecast_qty_2 ?? 1}}"
                                                name="products[{{$detail->product->id ?? ''}}][forecast_qty_2]"></td>
                                        <td><input class="form-control" type="number" value="{{$detail->forecast_qty_3 ?? 1}}"
                                                name="products[{{$detail->product->id ?? ''}}][forecast_qty_3]"></td>
                                        <td class="text-start"><button class="btn btn-danger remove-row"><i class="fas fa-trash-alt"></i></button></td>
                                    </tr>
                                @endforeach
                        </tbody>
                    </table>
